...
<c:set var="series" value="${qaf:getQAFSeriesByKey(current.qafSeriesBKey)}" scope="page"/>
<c:if test="${empty selectedFilm}">
  <c:set var="selectedFilm" >${qaf:getFeaturedFilmByKey(current.featuredFilmBKey)}</c:set>
</c:if>
<!DOCTYPE html>
<html> 
  <head>
